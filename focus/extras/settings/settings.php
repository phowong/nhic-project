...
function siteorigin_settings_enqueue_scripts( $prefix ) {
	...
	wp_localize_script( 'siteorigin-settings', 'siteoriginSettings', array(
		'premium' => array(
			...
			'name' => apply_filters('siteorigin_premium_theme_name', ucfirst( get_option( 'template' ) ) . ' ' . __( 'Premium', 'focus' ) ),
		)
	) );
	...
	switch ( $args['type'] ) {
		...
		case 'widget' :
			...
			if( !class_exists($args['widget_class']) ) {
				...
				printf( __('This field requires the %s plugin. ', 'focus'), $args['plugin_name']);
				if( function_exists('siteorigin_plugin_activation_install_url') ) {
					...
					printf( __('<a href="%s" target="_blank">Install %s</a> now. ', 'focus'), $install_url, $args['plugin_name']);
				}
				?></div>
...
function siteorigin_settings_add_slider_options($options){
	...
	if( class_exists('MetaSliderPlugin') ){
		...
		foreach($sliders as $slider) {
			$options['[metaslider id="'.$slider->ID.'"]'] = __('Meta Slider: ', 'focus').$slider->post_title;
		}
	}
	...
	if( function_exists('rev_slider_shortcode') ) {
		...
		foreach($sliders as $slider) {
			$options['[rev_slider '.$slider->alias.']'] = __('Revolution Slider: ', 'focus').$slider->title;
		}
	}
	...
	if( function_exists('layerslider') ) {
		...
		foreach($sliders as $slider) {
			$options['[layerslider id="'.$slider->id.'"]'] = __('LayerSlider: ', 'focus').$slider->name;
		}
	}
